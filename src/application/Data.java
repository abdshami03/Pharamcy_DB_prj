...
public class Data extends Application {
	...
	public void start(Stage primaryStage) throws IOException {
		...
		primaryStage.setTitle("Battir pharmacy");
	...
	private static void connectDB() throws ClassNotFoundException, SQLException {
		...
		p.setProperty("password", "1210591");
...
// data customer from sql and number of alll customer
	private static void getDataCustomer() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			...
			dataCustomer.add(new Customer(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getString(4), rs.getLong(5),
					rs.getInt(6)));
			Cust_count++;
	...
	private static void getDataPharmacist() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			dataPharmacist.add(new Pharmacist(Integer.parseInt(rs.getString(1)), rs.getString(2),
	...
	public void getq() {
		...
		try {
			...
			stmt.execute(SQLMode);

			String SQL = "SELECT M.mid, m.mtype AS medicine_name, SUM(im.quantity) AS total_price "
					+ "FROM Medicine M, Invoice I, Invoice_med im " + "WHERE M.mid = Im.mid AND im.invid = i.vid "
					+ "GROUP BY M.mid, m.mtype " + "ORDER BY total_price DESC " + "LIMIT 0, 1000;";

			rs = stmt.executeQuery(SQL);

			while (rs.next()) {
				medicineResults.add(new sales(rs.getString("medicine_name"), rs.getInt("total_price")));
			}
		} catch (SQLException e) {
	...
	public void graphing() throws SQLException, ClassNotFoundException {
		...
		try {
			...
			String SQL = "SELECT im.sdate, SUM(M.Price * im.quantity) AS sales "
					+ "FROM medicine M, invoice_med im, invoice i " + "GROUP BY im.sdate " + "ORDER BY im.sdate;";
			rs = stmt.executeQuery(SQL);
	...
	private static void getDataInsurance() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			...
			dataInsurance.add(
					new Insurance(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
		}
		rs.close();
		stmt.close();
		con.close();
	...
	private static void getDataMedicine() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			...
			dataMedicine.add(new Medicine(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
					Float.parseFloat(rs.getString(5)), rs.getInt(6), rs.getInt(7)));
		}
		rs.close();
		stmt.close();
		con.close();
	...
	private static void getDataPrescription() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			...
			dataPrescription.add(new Prescription(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getString(4),
	...
	private static void getDataInvoice_med() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			...
			dataInvoice_med
					.add(new Invoice_med(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
		}
		rs.close();
		stmt.close();
		con.close();
	...
	public static void ExecuteStatement(String SQL)
			throws SQLException, DateTimeParseException, IllegalArgumentException {
		try {
			Statement stmt = con.createStatement();
	...
	public static void insertData(Insurance ins) {
		try {
			System.out.println("insert into Insurance( instype , company , coverageSdate , coverageEdate) values("
					+ ins.getInstype() + ins.getCompany() + ins.getCoverageSdate() + ins.getCoverageEdate() + ");");
			connectDB();
			ExecuteStatement("insert into Insurance( instype , company , coverageSdate , coverageEdate) values('"
					+ ins.getInstype() + "','" + ins.getCompany() + "','" + ins.getCoverageSdate() + "','"
	...
	public static void insertData(Pharmacist ph) {
		try {
			System.out
					.println("Insert into Pharmacist(phname, age, gender, contact_info) values(" + "'" + ph.getPhname()
							+ "'," + ph.getAge() + ",'" + ph.getGender() + "','" + ph.getContact_info() + "');");

			connectDB();
			ExecuteStatement("INSERT INTO Pharmacist (phname, age, gender, contact_info) VALUES ('" + ph.getPhname()
					+ "', " + ph.getAge() + ", '" + ph.getGender() + "', '" + ph.getContact_info() + "');");
	...
	public static void insertData(Doctor d) {
		try {
			System.out.println(
					"Insert into Doctor(dname, specialty) values('" + d.getDname() + "', '" + d.getSpecialty() + "');");
			connectDB();
			ExecuteStatement("INSERT INTO Doctor (dname, specialty) VALUES ('" + d.getDname() + "', '"
					+ d.getSpecialty() + "');");
	...
	public static void insertData(Manufacturer m) {
		try {
			System.out.println("insert into Manufacturer( address , mname , phone) values(" + m.address + m.mname
					+ m.phone + ");");
			connectDB();
			ExecuteStatement("insert into Manufacturer( address , mname , phone) values('" + m.address + "','" + m.mname
					+ "'," + +m.phone + ");");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void insertData(Medicine m) {
		...
		try {
			System.out
					.println("insert into Medicine(mid , mType , dosage , expiryDate , price , quantity , cid ) values("
							+ m.mid + ",'" + m.mType + "','" + m.dosage + "','" + m.expiryDate + "'," + m.price + ","
							+ m.quantity + "," + m.cid + ");");
			connectDB();
	...
	public static void insertData(Prescription p) {
		...
		try {
			System.out.println("insert into Prescription(did , cusid , mid , per_text , per_date , quantity) values("
					+ p.did + "," + p.cusid + "," + p.mid + ",'" + p.per_text + "','" + p.per_date + "'," + p.quantity
					+ ");");
			connectDB();
	...
	public static void insertData(Invoice i) {
		...
		try {
			System.out.println("insert into Invoice(paymentMeth , pharid , cid ) values('" + i.paymentMeth + "',"
					+ i.pharid + "," + i.cid + ");");
			connectDB();
			ExecuteStatement("insert into Invoice( paymentMeth ,pharid , cid) values('" + i.paymentMeth + "',"
					+ i.pharid + "," + i.cid + ");");
			con.close();
			System.out.println("Connection Closed");
			dataInvoice.clear();
	...
	public static void insertData(Invoice_med im) {
		...
		try {
			System.out.println("insert into Invoice_med(invid, mid, cid ,  quantity) values(" + im.invid + "," + im.mid
					+ "," + im.cid + "," + im.quantity + ");");
			connectDB();
			ExecuteStatement("insert into Invoice_med(invid, mid ,cid , quantity,sdate) values(" + im.invid + ","
					+ im.mid + "," + im.cid + "," + im.quantity + "," + im.sdate + ");");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void insertData(Customer c) {
		...
		try {
			System.out.println("Insert into Customer(  cname, age , gender , phone , insid) values('" + c.getCname()
					+ "'," + c.getAge() + ",'" + c.getGender() + "'," + c.getPhone() + "," + c.getInsid() + ");");

			connectDB();
			ExecuteStatement("Insert into Customer( cname, age , gender , phone , insid) values('" + c.getCname() + "',"
					+ c.getAge() + ",'" + c.getGender() + "'," + c.getPhone() + "," + c.getInsid() + ");");
			con.close();
			System.out.println("Connection closed");
			dataCustomer.clear();
	...
	public static void deleteRow(Prescription row) {
		try {
			System.out.println("delete from Prescription where mid = " + row.mid + " and cusid = " + row.cusid
					+ " and did =  " + row.did + ";");
			connectDB();
			ExecuteStatement("delete from Prescription where mid = " + row.mid + " and cusid = " + row.cusid
					+ " and did =  " + row.did + ";");
			con.close();
			System.out.println("Connection closed");
			getdata();
	...
	public static void deleteRow(Invoice_med row) {
		try {
			System.out.println("delete from Invoice_med where invid = " + row.invid + " and mid = " + row.mid
					+ " and cid = " + row.cid + ";");
			connectDB();
			ExecuteStatement("delete from Invoice_med where invid = " + row.invid + " and mid = " + row.mid
					+ " and cid = " + row.cid + ";");
			con.close();
			System.out.println("Connection closed");
			getdata();
	...
	public static void updateInsuranceCoverageSdate(int insid, String coverageSdate) {
		try {
			...
			connectDB();
			ExecuteStatement(
					"update Insurance set coverageSdate = '" + date.toString() + "'  where insid = " + insid + ";");
			con.close();
			System.out.println("Connection Closed");
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public static void updateInsuranceCoverageEdate(int insid, String coverageEdate) {
		try {
			...
			System.out.println(
					"update Insurance set coverageEdate = '" + coverageEdate + "' where insid = " + insid + ";");
			connectDB();
	...
	// Update Manufacturer Attributes:
	...
	public static void updateMedicineType(int mid, int cid, String mType) {
		try {
			connectDB();
			System.out.println(
					"update Medicine set mType = '" + mType + "' where mid = " + mid + " and cid = " + cid + ";");
			ExecuteStatement(
					"update Medicine set mType = '" + mType + "' where mid = " + mid + " and cid = " + cid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void updateMedicineDosage(int mid, int cid, String dosage) {
		try {
			connectDB();
			System.out.println(
					"update Medicine set dosage = '" + dosage + "' where mid = " + mid + " and cid = " + cid + ";");
			ExecuteStatement(
					"update Medicine set dosage = '" + dosage + "' where mid = " + mid + " and cid = " + cid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void updateMedicineExpiryDate(int mid, int cid, String expiryDate) {
		try {
			connectDB();
			System.out.println("update Medicine set expiryDate = '" + expiryDate + "' where mid = " + mid
					+ " and cid = " + cid + ";");
			ExecuteStatement("update Medicine set expiryDate = '" + expiryDate + "' where mid = " + mid + " and cid = "
					+ cid + ";");
			con.close();
			System.out.println("Connection Closed");
		} catch (SQLException e) {
	...
	public static void updateMedicinePrice(int mid, int cid, float price) {
		try {
			connectDB();
			System.out.println(
					"update Medicine set price = " + price + " where mid = " + mid + " and cid = " + cid + ";");
			ExecuteStatement(
					"update Medicine set price = " + price + " where mid = " + mid + " and cid = " + cid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void updateMedicineQuantity(int mid, int cid, int quantity) {
		try {
			connectDB();
			System.out.println(
					"update Medicine set quantity = " + quantity + " where mid = " + mid + " and cid = " + cid + ";");
			ExecuteStatement(
					"update Medicine set quantity = " + quantity + " where mid = " + mid + " and cid = " + cid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void updatePrescriptionText(int did, int cusid, int mid, String per_text) {
		try {
			connectDB();
			System.out.println("update Prescription set per_text = '" + per_text + "' where did = " + did
					+ " and cusid = " + cusid + " and mid = " + mid + ";");
			ExecuteStatement("update Prescription set per_text = '" + per_text + "' where did = " + did
					+ " and cusid = " + cusid + " and mid = " + mid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void updatePrescriptionDate(int did, int cusid, int mid, String per_date) {
		try {
			connectDB();
			System.out.println("update Prescription set per_date = '" + per_date + "' where did = " + did
					+ " and cusid = " + cusid + " and mid = " + mid + ";");
			ExecuteStatement("update Prescription set per_date = '" + per_date + "' where did = " + did
					+ " and cusid = " + cusid + " and mid = " + mid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	public static void updatePrescriptionQuantity(int did, int cusid, int mid, int quantity) {
		try {
			connectDB();
			System.out.println("update Prescription set quantity = " + quantity + " where did = " + did
					+ " and cusid = " + cusid + " and mid = " + mid + ";");
			ExecuteStatement("update Prescription set quantity = " + quantity + " where did = " + did + " and cusid = "
					+ cusid + " and mid = " + mid + ";");
			con.close();
			System.out.println("Connection Closed");
	...
	// Update Attributes of Invoice_med:

	public void updateInvoice_medQuantity(int invid, int mid, int cid, int quantity) {
		try {
			connectDB();
			System.out.println("update Invoice_med set quantity = " + quantity + " where mid = " + mid + " and cid = "
					+ cid + " and invid = " + invid + ";");
			ExecuteStatement("update Invoice_med set quantity = " + quantity + " where mid = " + mid + " and cid = "
					+ cid + " and invid = " + invid + ";");
			con.close();
			System.out.println("Connection Closed");
