...
public class mysales implements Initializable {
	...
	public void addCart(javafx.event.ActionEvent actionEvent) {
		...
		try {
			...
			if (selectedItem != null) {
				...
				if (check == true)
					allCart.add(new Cart(selectedItem.mType, selectedItem.price, Q, selectedItem.mid, selectedItem.cid,
	...
	public void Buy(javafx.event.ActionEvent actionEvent) throws SQLException, ClassNotFoundException, IOException {
		...
		if (selectedDoctor == null || selectedCustomer == null || CustomerTXT == null || PharmasistCOMBO == null
				|| PaymentCOMBO == null || date == null) {
		...
		for (int i = 0; i < allCart.size(); i++) {
			if (allCart.get(i).oldq - allCart.get(i).quantity > 0) {
				Data.updateMedicineQuantity(allCart.get(i).id, allCart.get(i).cid,
						allCart.get(i).oldq - allCart.get(i).quantity);
				shouldContinue = true;
			}
		...
		Data.getdata();
		for (int i = 0; i < allCart.size(); i++) {
			Data.insertData(new Invoice_med(Data.countin, allCart.get(i).id, allCart.get(i).cid,
					allCart.get(i).quantity, String.valueOf(date.getValue())));
